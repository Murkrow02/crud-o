...
abstract class CrudoField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (!visible ||
        (visibleOn != null &&
            !visibleOn!.contains(formContext.operationType))) {
      return const SizedBox();
    }

    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 8.0),
      child: formContext.operationType == ResourceOperationType.view
          ? buildViewField(
              context,
              label ?? name,
              formContext.formData[name]?.toString() ?? '',
            )
          : buildField(context),
    );
  }
  ...
  {
    return CrudoViewField(name: name, value: value);
  }
}
